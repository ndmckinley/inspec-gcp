...
describe google_compute_disk({project: 'chef-gcp-inspec', name: '', zone: 'zone'}) do
  it { should exist }
  # Test that the image is the most recent image for the family
  its('source_image') { should match most_recent_image.self_link }
  its('type') { should match '' }
end

describe.one do
  google_compute_disk({project: 'chef-gcp-inspec', name: '', zone: 'zone'}).labels.each_pair do |key, value|
    describe key do
