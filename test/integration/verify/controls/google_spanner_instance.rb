...
control 'google_spanner_instance-1.0' do
  ...
  describe google_spanner_instance(project: gcp_project_id, name: spannerinstance['name'], config: spannerinstance['config']) do
    it { should exist }
    its('config') { should eq projects/gcp_project_id/instanceConfigs/spannerinstance['config'] }
    its('name') { should eq spannerinstance['name'] }
    its('display_name') { should eq spannerinstance['display_name'] }
    its('node_count') { should eq spannerinstance['node_count'] }
    its('labels') { should eq spannerinstance['labels'] }
  end
end
